...
from nt_filter import (AirBoundFilter, SearchEngineFilter, filter_airbounds,
                       filter_prices, filter_search_engine)
from nt_models import AirBound, CabinClass, PriceFilter
from nt_parser import (convert_aa_response_to_models,
                       convert_ac_response_to_models,
                       convert_dl_response_to_models, results_to_dash_table)
    ...
    def __init__(self):
        ...
        self.dash_app.layout = html.Div(
            [
                    ...
                    dbc.Stack(
                        [
                            ...
                            dbc.Input(
                                ...
                                value="TYO",
                                placeholder=
                                "Destination IATA code, support comma separated multiple destinations",
                            ),
                        ],
                        direction="horizontal",
                    ),
                ...
                dbc.Label(
                    "The system is to search EVERYDAY of the daterange you pick, only ONE-WAY.",
                    style={
                        "color": "Red",
                        "font-size": 20
                    },
                ...
                html.Div([
                    html.A(
                        "NT-tool is powered by an opensource library",
        ...
        def run_thread(searcher, converter, origins, destinations, dates,
                       airbounds):
            for ori in origins:
        ...
        def search_results(n_clicks, origins, destinations, start_date,
                           end_date):
            ...
            destinations = [
                "".join(des.split()) for des in destinations.split(",")
            ]
        ...
        def apply_filter(search_data, filter_options):
            ...
            price_filter = PriceFilter(
                ...
                preferred_classes=[
                    CabinClass[x] for x in filter_options["cabin_class"]
                ],
        ...
        def get_filter_options(n_clicks, cabin_class, search_engine,
                               airline_include, airline_exclude):
            return {

...
class PriceFilter(BaseModel):
    min_quota: int = 1
    preferred_classes: List[CabinClass] = [
        CabinClass.F,
        CabinClass.J,
        CabinClass.W,
        CabinClass.Y,
...
class AirBound(BaseModel):
    ...
    def calculate_from_to(segments: List[Segment], **kwargs):
        ...
        if len(segments) > 1:
            for x in range(1, len(segments)):
                connection = (
                    segments[x].excl_departure_time - segments[x - 1].excl_arrival_time
                )
    ...
    def to_flatted_list(self):
        excl_dict = {
            ...
            "excl_arrival_time": True,
            "price": {
                "__all__": {"excl_cash_in_base_unit", "excl_currency", "excl_miles"}
            },
            "segments": False,
        }
